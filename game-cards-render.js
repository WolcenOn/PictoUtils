(function () {
  ...
  function cardPath(ctx, w, h, shape, radius, inset) {
    ...
    ctx.beginPath();
    if (shape === "hex") {
      polygonPath(ctx, G.hexVertices(w, h, m));
      return;
    }
    if (shape === "oct") {
      polygonPath(ctx, G.octVertices(w, h, m));
      return;
    }
    if (shape === "oval") {
    ...
    const rw = Math.max(0, w - m * 2);
    const rh = Math.max(0, h - m * 2);
  ...
  G.loadImage = function (url) {
    ...
    if (G.imageCache.has(url)) return G.imageCache.get(url);
  ...
  async function drawPlacement(ctx, p, scale) {
    ...
    if (p.kind === "text") {
      const fontPx = (p.fontPx / G.MM_TO_TEXT_PX) * scale;
  ...
  G.renderCard = async function (card, scale) {
    ...
    // El contorno se dibuja medio grosor hacia dentro. El tamaño físico de la
    // tarjeta no cambia, pero ninguna parte del trazo queda fuera del canvas.
  ...
  G.downloadCanvas = function (canvas, filename) {
    canvas.toBlob((blob) => {
      if (!blob) return;
  ...
  G.downloadAllPng = async function () {
    for (const card of G.cards) {
      G.downloadCanvas(await G.renderCard(card, G.PDF_SCALE), `tarjeta_juego_${card.index + 1}.png`);
      await new Promise((r) => setTimeout(r, 120));
    }
  };

  function pdfFormat() {
    if (typeof cfg === "undefined") return "a4";
    if (String(cfg.pageSize || "a4").toLowerCase() === "custom") {
      return [Math.max(50, parseFloat(cfg.customWidth) || 210), Math.max(50, parseFloat(cfg.customHeight) || 297)];
    }
    return String(cfg.pageSize || "a4").toLowerCase();
  }

  function rectangularPdfSlots(pageW, pageH, W, H, margin, gap) {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        if (x + W <= pageW - margin + .01 && y + H <= pageH - margin + .01) slots.push({ x, y });
      }
    }
    return slots.length ? slots : [{ x: margin, y: margin }];
  }

  function honeycombPdfSlots(pageW, pageH, W, H, margin, gap) {
    const xStep = W * .75 + gap;
    const yStep = H + gap;
    const slots = [];
    let col = 0;
    for (let x = margin; x + W <= pageW - margin + .01; x += xStep, col++) {
      const offsetY = (col % 2) * yStep / 2;
      for (let y = margin + offsetY; y + H <= pageH - margin + .01; y += yStep) {
        slots.push({ x, y });
      }
    }
  ...
  G.downloadPdf = async function () {
    if (!G.cards.length || !window.jspdf?.jsPDF) return;
    const orientation = (typeof cfg !== "undefined" && cfg.orientation) ? cfg.orientation : "portrait";
    const pdf = new window.jspdf.jsPDF({ unit: "mm", orientation, format: pdfFormat() });
    const pageW = pdf.internal.pageSize.getWidth(), pageH = pdf.internal.pageSize.getHeight();
    const W = G.cfg.widthMm, H = G.cfg.heightMm;
    const normalGap = (typeof cfg !== "undefined" && Number.isFinite(parseFloat(cfg.gap))) ? Math.max(2, parseFloat(cfg.gap)) : 5;
    const isHex = G.cfg.shape === "hex";
    const hexMode = G.cfg.hexPrintMode || "honeycomb";
    const hexGap = Math.max(0, Number.isFinite(parseFloat(G.cfg.hexPrintGapMm)) ? parseFloat(G.cfg.hexPrintGapMm) : 2);
    const margin = Math.max(6, Number.isFinite(parseFloat(G.cfg.hexPrintPageMarginMm)) ? parseFloat(G.cfg.hexPrintPageMarginMm) : 9);
    const slots = isHex && hexMode === "honeycomb"
      ? honeycombPdfSlots(pageW, pageH, W, H, margin, hexGap)
      : rectangularPdfSlots(pageW, pageH, W, H, margin, isHex ? hexGap : normalGap);
    const perPage = Math.max(1, slots.length);

    for (let i = 0; i < G.cards.length; i++) {
      const pos = i % perPage;
      if (i > 0 && pos === 0) pdf.addPage();
      const slot = slots[pos] || slots[0];
      const canvas = await G.renderCard(G.cards[i], G.PDF_SCALE);
      pdf.addImage(canvas.toDataURL("image/png"), "PNG", slot.x, slot.y, W, H, undefined, "FAST");
    }
